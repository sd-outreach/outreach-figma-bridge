 ...
 *   node .cursor/specs/validate-spec.cjs .cursor/specs/  # validates all specs in directory
 * 
 * Exit codes:
 *   0 = all valid
...
const VALID_AXIS_SIZING = ['FIXED', 'AUTO'];
...
function validateNode(node, path, errors, warnings, parentHasAutoLayout = false) {
  ...
  if (node.autoLayout) {
    ...
    if (node.autoLayout.primaryAxisSizingMode && !VALID_AXIS_SIZING.includes(node.autoLayout.primaryAxisSizingMode)) {
      errors.push(`${loc}: Invalid autoLayout.primaryAxisSizingMode "${node.autoLayout.primaryAxisSizingMode}". Must be: ${VALID_AXIS_SIZING.join(', ')}`);
    }
    if (node.autoLayout.counterAxisSizingMode && !VALID_AXIS_SIZING.includes(node.autoLayout.counterAxisSizingMode)) {
      errors.push(`${loc}: Invalid autoLayout.counterAxisSizingMode "${node.autoLayout.counterAxisSizingMode}". Must be: ${VALID_AXIS_SIZING.join(', ')}`);
    }
  }
