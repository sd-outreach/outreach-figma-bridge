...
>
> **AUDIT / PACKAGE EXCEPTION: The `design_system_audit` and `generate_library_package` prompts are EXEMPT from the MODE GATE. When running either prompt, do NOT read `design-system-config.mdc`, do NOT check library packages or resolved variables, do NOT reference any component key mapping or token file, and do NOT follow any mode-specific rules from this file. Both prompts fetch ALL data directly from the connected Figma file via live MCP calls. The only rule from this file that still applies is RULE ZERO (Connection Gate) — the Figma connection must be verified before making MCP calls.**
